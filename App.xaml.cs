...

{
...

    {
...

        {
            ...
            ScheduleDeferredSilentAutoUpdate(mainWindow, e.Args);
    ...
    private async Task RunDeferredSilentAutoUpdateAsync(MainWindow mainWindow, string[] startupArguments)

    {
...

        {
            ...
            var shouldExit = await TryApplySilentAutoUpdateAsync(mainWindow, startupArguments).ConfigureAwait(true);

            if (!shouldExit || IsShuttingDown)
            ...
            StartupDiagnostics.Log("Deferred silent auto-update relaunch sequence initiated.");

            mainWindow.ReportAutoUpdateStatus(Loc.Get("Str_AutoUpdateRestartSoon"));
...

        {
            ...
            StartupDiagnostics.Log("Deferred silent auto-update failed open (bypass).", ex);
